...
void accessL1(uint32_t address, uint8_t *data, uint32_t mode) {
  ...
  if (SimpleCache.init == 0) {
    for (index = 0; index < BLOCK_SIZE; index++) { // Go through every line in the block
        SimpleCache.lines[index].Valid = 0;
    }
    SimpleCache.init = 1;
  }

  // TODO: Find the correct line, aka index bits
  CacheLine *Line = SimpleCache.lines;

  Tag = address >> 3; // Why do I do this?

  MemAddress = address >> 3; // again this....! (Tomas): Tirar o offset ?????
